...
#### data "aws_ami"
Earlier I mentioned there are different types of blocks. So far we've only used resource blocks. Resource blocks command Terraform to create new physical infrastructure. a **data** block is entirely different. It instructs Terraform to perform a read-only query against the AWS API to get existing information. 
...
![Star Wars It Is Done](images/starwars.gif)

### Deployment Artifacts

**Terminal Output:**
![Terraform Apply Success](images/Phase_2_Tlab_05_Apply_complete.png)

**S3 Secure Vault:**
![Private S3 Bucket](images/Phase_2_Tlab_05_S3_Bucket.png)

**EC2 Compute Identity:**
![EC2 IAM Role](images/phase_2_Tlab_05_EC2_instance.png)

**Destroy Complete:**
![Terraform Destroy Complete](images/Phase_2_Tlab_05_Destroy_Complete.png)

# Sike
![Tyler Sike](images/sike-sikes.gif)
...
**Boom Destroy**
![Kumamon Boom](images/kumamon_boom.jpg)

## References 

* [Scholarly article on Denial of Wallet attacks and its threat to serverless computing](https://www.sciencedirect.com/science/article/pii/S221421262100079X)
* [Explains what Single Sign on (SSO) is](https://www.cloudflare.com/learning/access-management/what-is-sso/)
* [Federated Access vs Single Sign on](https://www.beyondtrust.com/resources/glossary/federated-access)
* [HashiCorp documentation explaining the difference between creating infrastructure (`resource`) and querying existing infrastructure (`data`).](https://developer.hashicorp.com/terraform/language/data-sources)
* [Explains modern Terraform syntax for referencing attributes from other blocks, creating implicit dependencies without needing the `${}` string interpolation unless combining text.](https://developer.hashicorp.com/terraform/language/expressions/references)
* [AWS documentation detailing why IAM Users are dangerous on EC2, and how Instance Profiles securely pass temporary credentials to applications running on the server](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/iam-roles-for-amazon-ec2.html)
* [AWS Security Blog post breaking down the "Principal" element and how trust policies act as the "bouncer" to dictate exactly who or what can assume a role.](https://aws.amazon.com/blogs/security/how-to-use-trust-policies-with-iam-roles/)
* [Defines what an AMI is (the OS blueprint) and why selecting the right architecture and version is necessary to launch an EC2 instance.](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/AMIs.html)
* [AWS best practices for securing S3 buckets, tying back to our IAM policy that only allows the `s3:PutObject` action.](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security-best-practices.html)
* [HashiCorp documentation explaining the purpose of Terraform state (`terraform.tfstate`), how it acts as a database mapping real-world resources to your configuration, and why losing it causes Terraform to "forget" existing infrastructure.](https://developer.hashicorp.com/terraform/language/state/purpose)
* [HashiCorp documentation on the Dependency Lock File (`.terraform.lock.hcl`), explaining how Terraform tracks provider plugin versions and why the `terraform init -upgrade` command is necessary when adding new providers like random.](https://developer.hashicorp.com/terraform/language/files/dependency-lock)
* [Official HashiCorp documentation for the `random_id` resource, used to generate cryptographically strong random identifiers to prevent global namespace collisions when creating S3 buckets.](https://registry.terraform.io/providers/hashicorp/random/latest/docs/resources/id)
* [AWS documentation on using AWS Budgets to set custom cost limits and track usage, serving as the primary financial guardrail against the Denial of Wallet attacks mentioned in the introductory concepts.](https://docs.aws.amazon.com/cost-management/latest/userguide/budgets-managing-costs.html)